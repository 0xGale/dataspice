...
`dataspice` makes it easier for you to get started documenting your data so other people can find and re-use it. This documentation (metadata) can also be used to:

- Create a website for your data (like [pkgdown](https://github.com/r-lib/pkgdown) for your data) that you can easily host on [GitHub Pages](https://pages.github.com/) or elsewhere
- Easily transition to a richer metadata standard to document your data even further
- Integrate with Google's [public datataset initiative](https://ai.googleblog.com/2017/01/facilitating-discovery-of-public.html) by automatically marking your website up with [Structured Data](https://developers.google.com/search/docs/guides/intro-structured-data)

The metadata `dataspice` helps you collect is based on [Schema.org/Dataset](http://schema.org/Dataset) with an eye towards exporting to richer [metadata standards](https://github.com/ropenscilabs/dataspice#resources).
...
You can install the development version from [GitHub](https://github.com/ropenscilabs/dataspice) with:

``` r
# install.packages("remotes")
remotes::install_github("ropenscilabs/dataspice")
```
## Workflow

```{r example, eval=FALSE}
create_spice() # Copy empty metadata templates into your repo
# Edit the templates in data/metadata
write_spice() # Writes out JSON-LD to data/metadata
build_site() # Create a static site in docs
```
